...
.controller("ScriptModController", function($scope, SisSession,
    ...
    var parseRoute = function() {
        ...
        var action = scriptId ? "edit" : "add";
        var schema = SisUtil.getScriptSchema();
        if (action == 'add' && !scriptId) {
            // adding..
            $scope.schema = schema;
            $scope.action = action;
            $scope.title = "Add a new script";
            var obj = SisSession.getObjectToCopy(schema.name) || { };
            obj._sis = { owner : [], tags : [] };
            init(obj);
        } else if (action == 'edit' && scriptId) {
            SisApi.getScript(scriptId).then(function(res) {
                $scope.schema = schema;
                $scope.action = action;
                $scope.title = "Modify script " + scriptId;
                init(res);
            }, function(err) {
                return $scope.$state.go("^.list");
            });
